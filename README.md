...
`official_prep.py` / `official_merge.py` are that selection pass for characters:
prep batches only the *ambiguous* tags — a language bucket holding more than one
candidate, since a bucket of one needs no judgement — and merge validates the
answers, reporting which batch indices came back missing or malformed so a run
...
`death_note` was the canonical case: its wiki lists only Japanese aliases, and its
Japanese title is katakana, so there was not even a Han name to convert from —
`デスノート` and `death note` found it, `死亡笔记` did not.
