...
#   回 又一条 jp2t 短语规则「回転->廻転」。整词换掉之后 転 就轮不到逐字转换了,
#     于是「回転少女」卡在那里,既不是日文也不是中文。摘出 回 就把词拆开,転->轉->转
...
#     (东方的九十九弁々)就是这么坏掉的
PROTECTED = frozenset("吒乾於薹徵紬芸疏予欠装醋露回樫魟鰤鯱鉾弁")
...
# OpenCC 给不出正确中文形的字形,在它插手之前先换掉。两种成因:
#   姫 -> 姬、姉 -> 姐:jp2t 原样返回,中文根本没有这两个字形
#   篠 -> 筱:t2s 会转,但转成生僻异体「筿」。标准写法是筱(筱田麻里子、秋筱宫),
#     而语料里两种并存过 —— 同一个人在 shinoda_mariko 是「筱田麻里子」(人工写的),
#     在 kishida_mimori 是「8代目筿田麻里子」(机器转的),63 条对 7 条
#   々 是日文叠字符号,现代中文不用:幽々子 -> 幽幽子
# 修掉这些能救 1,829 条已发布名字,最热的是 saigyouji_yuyuko(27,787 投稿)的
